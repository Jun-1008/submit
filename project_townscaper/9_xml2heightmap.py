...
def xml2heightmap(save_path, xml_path):
    ...
    # 学習データのxml
    for xml in xmls:
        # ファイル名だけなのでパスをつなげる必要あり
        xml_file = os.path.join(xml_path, xml)
        tree = ET.parse(xml_file) # XMLファイルオープン。これは学習データの大元
        root = tree.getroot()

        for j in range(15): # 建物は最高14階
            # with open (csv_file, 'w', newline='') as f0: #書き込み用csv
            with open (os.path.join(save_path, xml.split('.')[0] + '-h' + str(j) + '.csv'), 'w', newline='') as f0: #書き込み用csv
...
if __name__ == '__main__':
    xml2heightmap('split_csv', 'xml_perlin')
